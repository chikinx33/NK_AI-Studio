...
function workReference(work: CompanyWorkItem): ChatReference {
  ...
  return {
    ...
    url: objectName ? mediaUrlFromOutput({ objectName }) : undefined,
    dateKey: koreaDate(work.created_at),
  };
}

export default function WorkExplorer({ revision = 0, initialDate = "", onOpenWork, onOpenProject, onChatAbout, onAddChatReference, chatReferenceKeys = [] }: { revision?: number; initialDate?: string; onOpenWork: (work: CompanyWorkItem) => void; onOpenProject: (projectId: string) => void; onChatAbout?: (ref: ChatReference) => void; onAddChatReference?: (ref: ChatReference) => void; chatReferenceKeys?: string[] }) {
