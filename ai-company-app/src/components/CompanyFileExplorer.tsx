  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-[#090d13]">
    ...
    <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-edge bg-[#0b1018] px-5 py-2.5">
      ...
      <div className="ml-auto flex flex-wrap items-center gap-2">
        ...
        <button type="button" onClick={() => void transfer("copy")} disabled={!selectedFileEntries.length || !!selectedWorkFolders.length || !!busy} className="rounded-lg border border-edge px-3 py-2 text-xs text-gray-300 disabled:opacity-30">복사</button>
        <button type="button" onClick={() => void transfer("move")} disabled={!selectedFileEntries.length || !!selectedWorkFolders.length || !!busy} className="rounded-lg border border-edge px-3 py-2 text-xs text-gray-300 disabled:opacity-30">이동</button>
    ...
    <main className="min-h-0 flex-1 overflow-y-auto p-5">
      {loading ? <div className="grid min-h-64 place-items-center text-sm text-gray-500">회사 파일을 불러오는 중…</div> : visibleEntries.length ? viewMode === "list" ?
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{visibleEntries.map((entry) => <article key={entry.path} className={`relative rounded-2xl border p-4 transition ${selected.has(entry.path) ? "border-emerald-600 bg-emerald-950/20" : "border-edge bg-panel hover:border-gray-600"}`}><input type="checkbox" checked={selected.has(entry.path)} onChange={() => toggle(entry.path)} className="absolute right-3 top-3 accent-emerald-500"/><button type="button" onClick={() => openEntry(entry)} className="block w-full text-left"><EntryIcon entry={entry}/><h2 className="mt-3 truncate text-xs font-bold text-gray-100" title={entry.name}>{entry.name}</h2><div className="mt-2 flex justify-between text-[10px] text-gray-500"><span>{entry.kind === "folder" || entry.kind === "work-folder" ? "폴더" : entry.contentType || "파일"}</span><span>{entry.kind === "file" ? formatBytes(entry.size) : entry.kind === "work-folder" ? `${entry.itemCount || 0}개` : ""}</span></div></button></article>)}</div> :
