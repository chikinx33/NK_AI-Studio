  ...
  const setupParentLogic = () => {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (isAiVideo || isAiImage) {
        ...
        const target = isAiVideo ? '/ai-video' : '/ai-image';
    ...
    document.addEventListener('click', (e) => {
      ...
      const isAiVideoLink = !!href && /\/ai-video(\.html)?(\?|$)/i.test(href);
      const isAiImageLink = !!href && /\/ai-image(\.html)?(\?|$)/i.test(href);
    ...
    const setApiSettingsCollapsed = (collapsed) => {
      ...
      if (apiSettingsToggleBtn) {
        ...
        apiSettingsToggleBtn.setAttribute(
          'aria-label',
          translateUiText(apiSettingsCollapsed ? 'API 연결 펼치기' : 'API 연결 접기')
        );
      }
    };
