/**
 * _shared/stage-geometry.js — 무대(세트) 좌표계와 카메라 방위 기하 (서버판, ESM).
 *
 * 브라우저 stage-geometry.js 와 표가 같아야 한다 — tests/prompt-assembly-parity.test.mjs 가 비교한다.
 * (prototype/js/service/stage-geometry.js 의 IIFE 를 그대로 옮긴 것. TRANSFORMS / FACING_TEXT 본문은
 *  글자 단위로 동일해야 하며, 한쪽만 고치면 패리티 테스트가 막는다.)
 *
 * 왜 서버에도 두는가: 에이전트(픽셀)가 씬 스틸/영상을 만들 때 브라우저와 똑같은 블로킹 문장을
 * 붙여야 "같은 컷을 두 경로로 만들었더니 공간 배치가 다르다"는 사고를 막을 수 있다.
 * 전부 순수 함수 — window/NK 전역을 참조하지 않는다.
 */

export const X_KEYS = ['left', 'center', 'right'];
export const DEPTH_KEYS = ['near', 'mid', 'far'];
export const FACING_KEYS = ['camera', 'away', 'left', 'right'];

export function mirrorX(x) { return x === 'left' ? 'right' : (x === 'right' ? 'left' : 'center'); }
export function invertDepth(d) { return d === 'near' ? 'far' : (d === 'far' ? 'near' : 'mid'); }

// 프레임 기준 시선 표현 (영문 프롬프트 조각)
export const FACING_TEXT = {
...
export function normalizeBlocking(value) {
...
export function transformEntry(entry, cameraDirection) {
...
export const FRAME_X_TEXT = { left: 'on the left of frame', center: 'at the center of frame', right: 'on the right of frame' };
export const FRAME_DEPTH_TEXT = { near: 'in the foreground close to camera', mid: 'at mid-distance', far: 'in the background far from camera' };
 ...
 */
export function buildBlockingLines(blocking, cameraDirection, allowedTokens) {
...
export const DIRECTION_VARIANT_IDS = { front: 'dir-front', back: 'dir-back', left: 'dir-left', right: 'dir-right' };

export function directionVariantId(cameraDirection) {
