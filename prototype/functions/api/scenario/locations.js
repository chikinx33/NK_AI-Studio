...
Goal: identify the episode's ACTUALLY-DISTINCT physical spaces and group scenes by them.

Rules:
- Merge the same space even if phrased differently per scene (e.g. "outdoor pool", "pool edge", "water filling the frame" are ONE pool).
- Camera/framing/momentary-state words ("low angle", "1:1 frame", "close-up", "splash", "dry basin") are NOT location names — ignore them, look at the essential place.
- Split ONLY truly different spaces (e.g. pool entrance vs underwater vs street). Do NOT over-split. An episode usually has 1-5 spaces.
- For each space, write a "background-plate description": the EMPTY environment only — architecture, structure, materials, colors, lighting, props — with NO characters, NO action, NO camera direction. One paragraph for image generation.
- Map each space to the scene indexes (#) that use it.
...
목표: 이 에피소드에서 "실제로 구분되는 물리적 공간"을 식별해 씬을 묶는다.

규칙:
- 같은 공간을 씬마다 다르게 묘사했어도 하나로 묶는다. (예: "야외 수영장", "수영장 가장자리", "물이 가득 찬 풀장"은 모두 하나의 수영장)
- "낮은 앵글", "1:1 프레임", "클로즈업", "물보라", "바닥이 드러난" 같은 카메라/프레이밍/순간 상태 표현은 공간 이름이 아니다 — 무시하고 본질적 장소만 본다.
- 진짜로 다른 공간(예: 수영장 입구 vs 수영장 물속 vs 길거리)만 분리한다. 과도하게 쪼개지 마라. 보통 한 에피소드는 1~5개 공간.
- 각 공간에 "배경 플레이트 묘사"를 쓴다: 캐릭터·동작·카메라 없이 그 공간의 빈 배경만(건축·구조·재질·색·조명·소품). 이미지 생성용 한 문단.
- 각 공간이 어느 씬(#번호)들에 쓰이는지 sceneIndexes 로 매핑한다.
