...
// GET = { ok, data: { title, iconDataUrl, companyTagline } } / POST = 보낸 필드만 갱신
...
async function readBrand(env: any, userId: string) {
  ...
  return { title: normalizeBrandTitle(src.title), iconDataUrl: normalizeBrandIcon(src.iconDataUrl), companyTagline: String(src.companyTagline ?? "").trim().slice(0, MAX_COMPANY_TAGLINE) };
}
...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(body || {}, "companyTagline")) {
      if (typeof body.companyTagline !== "string" || !body.companyTagline.trim() || body.companyTagline.trim().length > MAX_COMPANY_TAGLINE) {
        return send({ error: "invalid_company_tagline", message: "회사 문구는 1~80자로 입력해 주세요." }, 400, origin);
      }
      next.companyTagline = body.companyTagline.trim();
    }
