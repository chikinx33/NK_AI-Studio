...
export const onRequestGet: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    if (res.status === 404) {
      return send({ ok: true, data: { userId, profile: {} } }, 200, origin);
    }
    if (!res.ok) return send({ error: text || "profile_get_failed" }, res.status, origin);
    const parsed = safeJson(text) || {};
    return send({ ok: true, data: { userId, profile: parsed.profile || parsed || {} } }, 200, origin);
  } catch (e: any) {
