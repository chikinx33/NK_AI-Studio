...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    const objectName = buildUserDataObject(basePrefix, userId, "profile.json");
    const payload = {
      userId,
      profile,
    ...
    const userProject =
      ...
      "";
    const uploadOnce = async (useUserProject: boolean) => {
      const uploadUrl = `https://storage.googleapis.com/upload/storage/v1/b/${encodeURIComponent(outParsed.bucket)}/o?uploadType=media&name=${encodeURIComponent(objectName)}${useUserProject && userProject ? `&userProject=${encodeURIComponent(userProject)}` : ""}`;
    ...
    if (!res.ok) return send({ error: text || "profile_save_failed" }, res.status, origin);
...
function sanitizeProfile(profile: any): Record<string, any> {
  const source = profile && typeof profile === "object" ? profile : {};
  const result: Record<string, any> = {};
  const entries = Object.entries(source).slice(0, 50);
  for (const [key, value] of entries) {
    const safeKey = String(key || "").trim().replace(/[^a-zA-Z0-9._-]/g, "_").slice(0, 64);
    if (!safeKey) continue;
    if (typeof value === "string") result[safeKey] = value.slice(0, 2000);
    else if (typeof value === "number" || typeof value === "boolean" || value === null) result[safeKey] = value;
    else if (Array.isArray(value) || typeof value === "object") result[safeKey] = value;
  }
  return result;
}
