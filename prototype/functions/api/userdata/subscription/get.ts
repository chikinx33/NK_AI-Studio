...
export const onRequestGet: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    const objectName = buildUserDataObject(basePrefix, userId, "subscription.json");
    ...
    const userProject =
      ...
      "";
    const dlUrl = `https://storage.googleapis.com/storage/v1/b/${encodeURIComponent(outParsed.bucket)}/o/${encodeURIComponent(objectName)}?alt=media${userProject ? `&userProject=${encodeURIComponent(userProject)}` : ""}`;
    ...
    if (res.status === 404) {
      return send({ ok: true, data: defaultSubscription(userId) }, 200, origin);
    }
    if (!res.ok) return send({ error: text || "subscription_get_failed" }, res.status, origin);
    const parsed = safeJson(text) || {};
    const data = {
      ...defaultSubscription(userId),
...
function defaultSubscription(userId: string) {
  return {
    userId,
    plan: "free",
    status: "inactive",
