...
export const onRequestPost: PagesFunction = async ({ request, env, params, waitUntil }) => {
  ...
  try {
    ...
    if (error instanceof CompanySkillJobTransitionError) {
      return send({ error: "실패한 업무만 다시 시도할 수 있습니다.", currentStatus: error.currentStatus }, 409, origin);
    }
    return send({ error: String(error?.message || error || "SkillJob 재시도 실패") }, 500, origin);
  }
};
