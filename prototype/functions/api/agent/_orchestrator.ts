...
export function buildAgentSystem(agentId: string, opts: BuildSystemOpts = {}): string {
  ...
  const MY_TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    image: `[[RUN: image | {"prompt": "이미지 설명 (구체적으로)", "aspectRatio": "16:9", "provider": "gemini|openai(선택)", "referenceImages": [{"imageUrl": "ip_library items[].signedUrl", "subjectDescription": "네모", "referenceKind": "character"}]}]]  → 이미지 생성. 기본은 서버 설정 모델(Gemini 3.1 Flash Image). 사용자가 "GPT로", "지피티 이미지로"처럼 지정하면 provider:"openai"(GPT Image)로 호출(막히면 Gemini로 자동 대체·결과에 실제 모델 표기). ★우리 캐릭터가 등장하는 그림이면 반드시 ip_library로 등록 시트를 먼저 조회해 referenceImages(최대 4개)로 넘긴다 — 넘기지 않으면 전혀 다른 캐릭터가 그려진다. 구도는 프롬프트가 정하고 시트는 캐릭터 디자인 유지에만 쓰인다.`,
    ...
    ip_library: `[[RUN: ip_library | {"brandId": "elidus"}]]  → 브랜드에 등록된 캐릭터/IP 자산(캐릭터 시트) 목록 조회. "우리 캐릭터", "스튜디오/브랜드 자산", "모양새 친구들처럼 등록된 캐릭터"는 전부 여기다. brandId를 모르면 brand_list로 먼저 확인하고, 브랜드가 하나면 그걸 쓴다. 결과 items[].signedUrl 과 파일명의 캐릭터 이름을 image의 referenceImages로 넘기면 캐릭터 디자인이 그대로 유지된다.`,
