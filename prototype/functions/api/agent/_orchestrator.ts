...
export function buildAgentSystem(agentId: string, opts: BuildSystemOpts = {}): string {
  ...
  const MY_TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    ip_describe: `[[RUN: ip_describe | {"character": "네모", "brandId": "선택"}]]  → IP 라이브러리에 **등록된 시트**를 브랜드 허브 맥락(세계관·규칙·금칙어·다른 캐릭터)과 함께 분석해 텍스트 속성 초안을 만든다. 돌려주는 값: description(생김새·긍정문), negativePrompt(안 나오게 할 것). ★"캐릭터 프롬프트 만들어줘/텍스트 속성 채워줘"는 이 도구다 — 시트를 눈으로만 보고 쓰면 이 IP 의 세계관을 모른 채 겉모습만 받아적게 된다. 초안일 뿐 저장되지 않는다.`,
