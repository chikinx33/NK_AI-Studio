...
export function buildAgentSystem(agentId: string, opts: BuildSystemOpts = {}): string {
  ...
  const MY_TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    brand_asset: `[[RUN: brand_asset | {"brandId": "my-brand", "name": "전략가", "kind": "character", "jobId": "그 캐릭터 이미지의 jobId(대화의 [산출물: … jobId=…])", "description": "생김새(선택)"}]]  또는 {"objectName": "저장소 경로(image_library 결과)"} · {"imageUrl": "이미지URL"}  → 이미지를 그 브랜드의 캐릭터(kind:character) 또는 환경(kind:background|prop) 자산으로 브랜드 허브에 등록. 한 번에 한 자산·한 이미지. 캐릭터가 여러 명이면 이름마다 따로 부르고 각자의 jobId/objectName 을 준다. 이미지를 지정하지 않으면 프롬프트에 그 이름이 들어간 최근 이미지를 찾고, 없으면 최근 이미지 목록(jobId)을 알려 주니 맞는 jobId 로 다시 부른다. 대화에 이미지가 없으면 image_library 로 objectName 을 먼저 찾는다. brandId 는 모르면 ip_library 로 확인(없는 브랜드는 만들지 않음). 이름은 영문·숫자·한글·_ 24자 이내, 캐릭터당 시트 4장·환경 자산 16개까지. ⚠️ 쓰기라 사람 승인 후 반영. "이 이미지를 우리 캐릭터 자산으로 등록해줘"에 사용.`,
