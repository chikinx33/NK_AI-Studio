...
export function buildAgentSystem(agentId: string, opts: BuildSystemOpts = {}): string {
  ...
  const MY_TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    brand_asset: `[[RUN: brand_asset | {"brandId": "my-brand", "name": "전략가", "kind": "character", "jobId": "방금 만든 이미지의 jobId(대화의 [산출물: … jobId=…])", "description": "생김새(선택)"}]]  또는 {"objectName": "저장소 경로"} · {"imageUrl": "이미지URL"}  → 이미지를 그 브랜드의 캐릭터(kind:character) 또는 환경(kind:background|prop) 자산으로 브랜드 허브에 등록. 이미지를 지정하지 않으면 가장 최근에 만든 이미지를 쓴다. brandId 는 모르면 ip_library 로 먼저 확인(없는 브랜드는 만들지 않음). 이름은 영문·숫자·한글·_ 24자 이내(공백은 _), 캐릭터당 시트 4장·환경 자산 16개까지. ⚠️ 쓰기라 사람 승인 후 반영. "이 이미지를 우리 캐릭터 자산으로 등록해줘"에 사용.`,
