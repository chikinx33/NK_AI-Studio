...
export function buildAgentSystem(agentId: string, opts: BuildSystemOpts = {}): string {
  ...
  const MY_TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    jobs_status: `[[RUN: jobs_status | {"limit": 20}]]  → 지금 내 작업들이 어디서 멈춰 있는지(사람 승인 대기 · 사람 검토 대기 · 진행 중 · 실패와 그 사유). ★사용자가 "왜 아직 안 나와?"·"결과가 없는데?"·"되고 있는 거 맞아?"라고 물으면 추측해서 답하지 말고 반드시 먼저 실행해 사실대로 알릴 것. 승인이 필요하면 무엇을 승인해야 하는지, 실패했으면 그 이유를 그대로 전한다.`,
  ...
  const TOOL_LABELS: Record<string, string> = {
    ...
    skill_job_retry: "파이프라인 재시도", skill_job_cancel: "파이프라인 취소", skill_job_continue: "파이프라인 이어하기",
