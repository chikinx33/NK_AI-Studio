...
export function buildAgentSystem(agentId: string, opts: BuildSystemOpts = {}): string {
  ...
  const MY_TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    publish: `[[RUN: publish | {"platforms": ["instagram"], "caption": "게시글 내용", "hashtags": ["태그1", "태그2"], "mediaUrl": "이미지/영상URL", "jobId": "지목한 산출물의 잡 ID(선택 · mediaUrl 대신 · [참조 산출물 … jobId=…])", "scheduledAt": "2026-07-10T19:00:00+09:00(선택·예약발행)"}]]  → 소유 SNS 채널에 발행. scheduledAt(ISO8601)을 주면 그 시각 예약발행. ★TikTok 은 바로 게시가 아니라 틱톡 앱 '초안함' 전송이다 — 사용자가 앱에서 공개 범위를 고르고 직접 게시한다. 제안·보고할 때 반드시 "초안함으로 보낸다"고 말하고 "게시했다"고 하지 말 것. TikTok 에는 예약이 없다(예약을 주면 TikTok 만 건너뛰고 안내). 영상은 jobId 또는 objectName 으로 지목한다. ⚠️ 항상 사람 승인 필요`,
...
프롬프트 문장을 직접 넣으려면 promptText(+promptEdited:true), 카메라 지시는 cameraDirection, 컷 길이는 estSec 를 쓴다. 컷마다 한 번씩 부른다(여러 컷을 한 번에 담지 않는다). ⚠️ 쓰기라 사람 승인 후 반영 — 승인 전에는 캔버스가 바뀌지 않는다.`,
  ...
  const TOOL_LABELS: Record<string, string> = {
    ...
    sns_analytics_sync: "SNS 성과 동기화", publish_history: "게시 이력 조회", tiktok_publish_status: "틱톡 발행 상태",
