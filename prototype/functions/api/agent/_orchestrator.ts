...
export function buildAgentSystem(agentId: string, opts: BuildSystemOpts = {}): string {
  ...
  const MY_TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    set_angle: `[[RUN: set_angle | {"projectId": "elidus-ep1", "locationName": "거실", "angle": "front"}]]  → 마스터(부감)를 소스로 같은 세트를 front|back|left|right|high|low 앵글로 다시 그림(카메라 재구성). 컷이 쓰는 앵글만 만든다. ⚠️ 크레딧 사용, 승인 후 실행.`,
