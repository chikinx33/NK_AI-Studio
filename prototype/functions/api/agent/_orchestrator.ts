...
export function buildAgentSystem(agentId: string, opts: BuildSystemOpts = {}): string {
  ...
  const MY_TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    company_files_read: `[[RUN: company_files_read | {"path": "폴더/파일.txt", "offset": 0, "limit": 12000}]]  → 업무 파일의 텍스트·JSON·CSV·Markdown·코드 내용을 읽고 채팅에 파일 열기 아이콘을 첨부(1MB 이하). 사용자가 '파일 보여줘/열어줘/읽어줘'라고 하면 반드시 실행. 확장자를 모르더라도 사용자가 말한 파일명을 path에 넣으면 서버가 단일 일치 파일을 찾음. 날짜 폴더 안의 파일은 목록에 나온 path(.work-files/날짜/파일.md)나 @work/날짜/파일.md 중 어느 쪽으로도 읽을 수 있음(업무 기록 자체는 파일이 아니라 못 읽음). hasMore=true이면 nextOffset을 offset으로 다시 호출해 끝까지 읽기.`,
