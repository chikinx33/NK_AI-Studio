...
export function buildAgentSystem(agentId: string, opts: BuildSystemOpts = {}): string {
  ...
  const MY_TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    project_create: `[[RUN: project_create | {"projectId": "elidus-ep1", "title": "ELIDUS"}]]  → 새 프로젝트(에피소드) 생성(GCS 폴더·초기화 + 표시 이름 지정). projectId는 폴더 식별자(영문/숫자/._-), title은 카드에 보이는 사람용 이름. ⚠️ title을 꼭 넣어야 카드가 기본값 "프로젝트"로 안 뜬다. 예) "ELIDUS 프로젝트 만들어" → {"projectId":"elidus","title":"ELIDUS"}. ⚠️ 쓰기라 사람 승인 후 생성.`,
  ...
  const TOOL_LABELS: Record<string, string> = {
    ...
    project_create: "프로젝트(에피소드) 생성", project_rename: "프로젝트 이름 변경", project_list: "프로젝트 목록 조회", project_get: "프로젝트 상태 조회",
