...
export const onRequestGet: PagesFunction = async ({ request, env }) => {
  try {
    ...
    if (!sessionId) return send({ error: "sessionId is required" }, 400, origin)
    ...
    const prefix = `${sessionPrefix}/outputs/`
    ...
    result.sort((a, b) => (new Date(b.updated || b.timeCreated).getTime() - new Date(a.updated || a.timeCreated).getTime()))
    return send({ items: result, prefix, sessionId }, 200, origin)
  } catch (e: any) {
...
async function signGcsUrl(opts: { bucket: string; object: string; clientEmail: string; privateKeyPem: string; expiresInSec: number; }) {
  ...
  const query = new URLSearchParams({ "X-Goog-Algorithm": "GOOG4-RSA-SHA256", "X-Goog-Credential": credential, "X-Goog-Date": time, "X-Goog-Expires": `${opts.expiresInSec}`, "X-Goog-SignedHeaders": signedHeaders })
