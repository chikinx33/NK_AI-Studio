 ...
 * 마무리한다. 그래서 post_info 를 보내지 않고, 확인 모달도 거치지 않는다
...
export const onRequestPost = async ({ request, env }: { request: Request; env: any }) => {
  ...
  try {
    ...
    // 영상 URL 확보 (서버가 직접 읽어 TikTok 에 올린다 — 공개 URL 노출 없음)
    let videoUrl: string;
    if (body.mediaGcsPath) {
      const { bucket } = parseGcsUri(env.VIDEO_OUTPUT_GCS_URI);
      videoUrl = await buildSignedUrl(
    ...
    // Direct Post 와 동일: total_chunk_count:1 통짜 전송이라 단일 청크 상한 64MB.
    ...
    if (videoSize > TIKTOK_MAX_SINGLE_CHUNK) {
      return send({
        error:
          ...
          `This video is ${Math.round(videoSize / 1024 / 1024)}MB. TikTok's single-chunk upload limit is 64MB.`,
        code: "video_too_large",
      }, 400);
    }

    // Step 1: 초안함 업로드 초기화. post_info 없음 — 게시 설정은 사용자가 앱에서 정한다.
    ...
    console.log(`[tiktok] inbox/video/init 응답 (${initRes.status}):`, JSON.stringify(initData));
