...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  try {
    ...
    if (!brandId || !confirm) return send({ error: "brandId and confirm=yes are required" }, 400, origin);
    ...
    let listedCount = 0;
    do {
      const listUrl = `https://storage.googleapis.com/storage/v1/b/${encodeURIComponent(parsed.bucket)}/o?prefix=${encodeURIComponent(brandPrefix)}&maxResults=1000${pageToken ? `&pageToken=${encodeURIComponent(pageToken)}` : ""}${userProject ? `&userProject=${encodeURIComponent(userProject)}` : ""}`;
      ...
      if (!res.ok) return send({ error: "List objects failed", status: res.status, detail: safeJson(text) }, res.status, origin);
      const json = safeJson(text);
      const items = Array.isArray((json as any).items) ? (json as any).items : [];
      listedCount += items.length;
      for (const it of items) {
        const name = String(it.name || "");
        if (!name.startsWith(brandPrefix)) continue;
        const delUrl = `https://storage.googleapis.com/storage/v1/b/${encodeURIComponent(parsed.bucket)}/o/${encodeURIComponent(name)}${userProject ? `?userProject=${encodeURIComponent(userProject)}` : ""}`;
    ...
    return send({ ok: true, brandId, prefix: brandPrefix, listedCount, deletedCount: results.filter((row) => row.status === 204).length }, 200, origin);
  } catch (e: any) {
