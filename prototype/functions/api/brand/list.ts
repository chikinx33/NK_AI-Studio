...
export const onRequestGet: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    const userId = resolveUserId(auth.userId, env);
    ...
    // delimiter 로 하위 폴더(브랜드 id) prefix 만 조회.
    const url = `https://storage.googleapis.com/storage/v1/b/${encodeURIComponent(parsed.bucket)}/o?delimiter=%2F&prefix=${encodeURIComponent(prefix)}&maxResults=500${userProject ? `&userProject=${encodeURIComponent(userProject)}` : ""}`;
    ...
    if (!res.ok && userProject && res.status === 403) {
      const urlNoUp = `https://storage.googleapis.com/storage/v1/b/${encodeURIComponent(parsed.bucket)}/o?delimiter=%2F&prefix=${encodeURIComponent(prefix)}&maxResults=500`;
      const res2 = await fetch(urlNoUp, { headers: { Authorization: `Bearer ${token}` } });
      const text2 = await res2.text();
      if (res2.ok) {
        text = text2;
      } else {
        return send({ error: "list_failed", detail: safeJson(text), hint: "serviceusage.services.use 권한 또는 GCS_BILLING_PROJECT_ID 설정 확인" }, res.status, origin);
      }
    } else if (!res.ok) {
