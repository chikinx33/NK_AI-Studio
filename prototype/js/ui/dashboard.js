; (function () {
  ...
  const getHostShell = () => {
    ...
    if (/\bpage-shell-ai-sound\b/.test(cls)) return 'sound';
    try {
      ...
      if (/ai-sound\.html/.test(url)) return 'sound';
    ...
    try {
      ...
      if (/ai-sound\.html/.test(ref)) return 'sound';
    ...
    try {
      ...
      if (h === 'brand' || h === 'video' || h === 'image' || h === 'videogen' || h === 'sound') return h;
    } catch (_) { }
    return 'video';
  };
  ...
  dashboard.renderDrafts = function () {
    ...
    container.onclick = (e) => {
      ...
      if (!btn && card) {
        ...
        if (host === 'sound') {
          // 자체 완결형 페이지: iframe 스테이지 대신 사운드 스튜디오 뷰로 직접 전환
          if (NK.uiSound && NK.uiSound.enterProject) NK.uiSound.enterProject(draft);
          return;
        }
        return;
      }
  ...
  dashboard.renderSidebarProjectCard = function (draft) {
    ...
    container.innerHTML = `
      ...
      <div class="sidebar-card-actions">
        ${getHostShell() === 'video' ? `<div class="sidebar-card-actions-row"><button class="btn-secondary sidebar-canvas-btn" data-action="sidebar-edit-canvas" data-i18n="sidebar_canvas_fixed">캔버스</button><button class="btn-secondary sidebar-previz-btn" data-action="sidebar-edit-previz" data-i18n="sidebar_previz_fixed">프리비즈</button></div>` : ''}
