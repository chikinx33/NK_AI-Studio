; (function () {
    ...
    var EN_TEXT_EXACT = {
        ...
        'AI 생성에 쓸 Claude 인증을 직접 등록합니다. 등록 전에는 AI 기능이 동작하지 않습니다.':
          'Register the Claude credentials used for AI generation. AI features stay disabled until you do.',
        '구독(OAuth)': 'Subscription (OAuth)',
        'API 키': 'API key',
        '터미널에서 claude setup-token 으로 발급한 구독 토큰(sk-ant-oat…)':
          'Subscription token issued by running claude setup-token in a terminal (sk-ant-oat…)',
        'Anthropic 콘솔에서 발급한 API 키(sk-ant-api…)': 'API key issued from the Anthropic console (sk-ant-api…)',
        '저장': 'Save',
        '인증 진단(라이브 테스트)': 'Test credentials (live call)',
        '불러오는 중…': 'Loading…',
        '저장 중…': 'Saving…',
        '진단 중…': 'Testing…',
        ...
        '적용 안 됨 — 이미지 생성·설명, 음악·효과음, 음성(TTS), 영상 생성, 지식 임베딩. 이 기능들은 OpenAI·Gemini·Kling 크레딧을 쓰므로 이 설정과 무관합니다.':
