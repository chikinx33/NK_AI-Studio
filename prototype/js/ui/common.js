; (function () {
    ...
    // linkedin·pinterest 는 제품 채널에서 빠졌지만, 과거 배포 이력 행이
    // 아이콘 없이 뜨지 않도록 남겨 둡니다(이력은 지우지 않습니다).
    ...
    common.platformIconSvg = function (platform, size) {
        var id = String(platform || '').trim().toLowerCase();
        if (id === 'youtube-shorts') id = 'youtube';
        var paths = PLATFORM_ICON_PATHS[id];
        if (!paths) return '';
        var dimension = Math.max(12, Number(size || 24) || 24);
        var attrs = id === 'instagram'
            ? 'fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"'
            : 'fill="currentColor"';
        return '<svg width="' + dimension + '" height="' + dimension + '" viewBox="0 0 24 24" ' + attrs + ' aria-hidden="true" focusable="false">' + paths + '</svg>';
    };
    ...
    var EN_TEXT_EXACT = {
        ...
        '로그인': 'Sign in',
        '로그아웃': 'Sign out',
        'Project 맥락': 'Project context',
        '내러티브 및 Tone 규칙': 'Narrative & Tone rules',
        '시각적 Style 규칙': 'Visual Style rules',
        '필수 지침': 'Mandatory directives',
        '로그인 중...': 'Signing in...',
        ...
        '더빙': 'Dubbing',
        '나레이션': 'Narration',
        '생성중(취소)': 'Generating (cancel)',
        '생성중': 'Generating',
        '영상중': 'Rendering',
        '이미지 생성 중...': 'Generating image...',
        '이미지 생성 실패': 'Image generation failed',
        '영상 생성 중...': 'Generating video...',
        '생성 실패': 'Generation failed',
        '자동 매핑': 'Auto-map',
        '이미지생성 모델': 'Image generation model',
        '내장 재생(임시 변환)': 'Inline playback (temp)',
        '프리프로덕션에서 나레이션/더빙을 켜야 음성 생성이 가능합니다.': 'Enable narration/dubbing in pre-production to generate voice.',
        '채팅·인페인팅으로 수정': 'Edit via chat / inpainting',
        '이전 버전으로 되돌리기': 'Revert to previous version',
        'Gemini · Kore (어린 소녀)': 'Gemini · Kore (Young girl)',
        'Gemini · Kore (어린 소년)': 'Gemini · Kore (Young boy)',
        'Gemini · Kore (로봇)': 'Gemini · Kore (Robot)',
        'Gemini · Kore (마법사)': 'Gemini · Kore (Wizard)',
        'Gemini · Kore (장난꾸러기)': 'Gemini · Kore (Trickster)',
      ...
      'OpenAI API 키 등록됨': 'OpenAI API key registered',
        ...
        '적용: 스튜디오·AI 기업의 텍스트와 이미지 전체': 'Applies to text and images across Studio and AI Company',
        '이미지 적용: AI 이미지·AI 문서 삽화·캐릭터/배경·장면 스틸·스토리보드·편집·카메라 변경·업스케일·AI 기업 에이전트.': 'Image coverage: AI Image, document illustrations, characters/backgrounds, scene stills, storyboards, editing, camera changes, upscaling and AI Company agents.',
        '음악·음성·영상 생성은 기존 설정을 사용합니다. 본인 인증 오류 시 마스터로 자동 전환하지 않습니다.': 'Music, speech and video generation retain their settings. Your credential errors never switch to master billing.',
        '이미지: 마스터 설정 사용': 'Images: using master settings',
        '이미지: 본인 OpenAI API 키 사용': 'Images: using your OpenAI API key',
        '이미지: 본인 OpenAI API 키 입력 필요': 'Images: enter your OpenAI API key',
        'ChatGPT(이미지) 구독 연결 정상': 'ChatGPT (images) subscription connected',
        '이미지: 연결 프로그램을 실행해 주세요': 'Images: start your connector',
        '이미지: 본인 ChatGPT 계정 연결 필요': 'Images: connect your ChatGPT account',
        ...
        '적용 안 됨 — 이미지 생성·설명, 음악·효과음, 음성(TTS), 영상 생성, 지식 임베딩. OpenAI·Gemini·Kling 크레딧을 쓰므로 이 설정과 무관합니다.':
          'Does not apply to — image generation and captioning, music and sound effects, speech (TTS), video generation, and knowledge embeddings. Those run on OpenAI, Gemini, or Kling credits and are unaffected by this setting.',
        'API 연결 펼치기': 'Expand API connection',
        'API 연결 접기': 'Collapse API connection',
        '저장': 'Save',
        '불러오는 중…': 'Loading…',
        '저장 중…': 'Saving…',
        '진단 중…': 'Testing…',
        ...
        '마스터 인증 사용 중 — 본인 사용 체크 시 전환': 'Using master credentials — check the box to use your account',
        ...
        ,'브랜드 전체 성과': 'Brand performance'
        ,'에피소드 성과': 'Episode performance'
        ,'분석 에피소드': 'Episode to analyze'
        ,'분석 범위': 'Analysis scope'
        ,'브랜드 전체 성과 목표': 'Brand performance goal'
        ,'에피소드 성과 목표': 'Episode performance goal'
        ,'게시물 귀속 확인': 'Post attribution'
        ,'임의 집계 금지': 'No automatic attribution'
        ,'대상 선택': 'Select destination'
        ,'브랜드 공통': 'Brand-wide'
        ,'이 브랜드와 관련 없음': 'Not related to this brand'
        ,'분류 저장': 'Save attribution'
        ,'적용 대상': 'Destination episode'
        ,'적용할 에피소드': 'Episode to receive draft'
        ,'에피소드를 선택해 주세요': 'Select an episode'
        ,'이 에피소드로 보내기': 'Send to this episode'
        ,'선택한 에피소드로 보내기': 'Send to selected episode'
    ...
    var EN_TOKEN_RULES = [
        ...
        { re: /광고/g, to: 'Ad' },
        { re: /컷 기반 생성/g, to: 'Reference cut' },
        { re: /컷 선택/g, to: 'Select cut' }
    ];
