; (function () {
    ...
    function normalizePublishResults(value) {
        ...
        return src.map(function (item, index) {
            ...
            return {
                ...
                brandId: normalizeText(raw.brandId),
                title: title,
                projectId: normalizeText(raw.projectId),
                projectTitle: normalizeText(raw.projectTitle || raw.episodeTitle),
                attributionStatus: normalizeText(raw.attributionStatus) || (raw.projectId || raw.brandId ? 'assigned' : 'unassigned'),
                attributionSource: normalizeText(raw.attributionSource),
                attributedAt: normalizeText(raw.attributedAt),
                note: note,
                caption: caption,
                hashtags: hashtags,
                metrics: {
    ...
    project.duplicate = async function (sourceId) {
        ...
        // consistent value and, more importantly, a later rename via the
        // dashboard pencil cannot be "undone" by a topic-based fallback.
        prePayload.episodeTitle = newTitle;
        prePayload.topic = newTitle;
