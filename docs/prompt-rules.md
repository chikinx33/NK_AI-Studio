...
## 3. 시나리오 프롬프트 구조
- **System**: 역할 분리, 태그별 관점 포함, 개요 톤과 브랜드 톤&매너의 우선순위 분리, Mandatory 우선, Scene 수/estSec 규칙, title/shot 필수, 마크다운 금지 등.
- **User**: Topic, Genre, Tags, Target, Needs, Overview Tone, Brand Tone & Manner, Style, Mandatory, AspectRatio, Duration를 라벨로 명시. 태그별 관점 문장 포함, Audience는 어휘/예시만 조정하도록 강조.
...
- **Common(공통)**: 전 프로젝트에 적용되는 시각 톤·룩·연출 가이드만 적는다.  
  ...
  - 톤 규칙: 브랜드 톤&매너는 고정 화법으로 유지하고, 개요 톤은 이번 영상의 정서 레이어로만 반영한다.
