...
- 현재 Brand Studio, Brand Hub, Brand Intelligence가 현재 선택된 `에피소드 draft/project` 기준으로 동작한다.
...
- 상단: 브랜드 요약과 현재 운영 컨텍스트만 짧게 표시
- 메인 좌측: 브랜드 자산 필터와 자산 선택, 콘텐츠 유형 선택
- 메인 우측: 캡션/해시태그 중심의 `발행 초안` 패널
- 채널 연결, 예약 게시, 게시 결과는 기본 노출이 아니라 접힘 섹션으로 분리
- 긴 자산 목록은 내부 스크롤로 제한해 문서 전체 스크롤을 과도하게 늘리지 않는다

### 7.2 Brand Hub

- 브랜드 공용 지식 저장
- 에피소드 개별 입력 화면이 아니라 브랜드 설정 화면으로 동작
- 정체성, 규칙, 참조/학습을 아코디언 구조로 나눠 기본 화면 높이를 줄인다
- 저장 구조 설명보다 실제 입력 필드를 우선 노출한다

### 7.3 Brand Intelligence

- 기본값은 브랜드 전체 성과 개요
- 에피소드별, 콘텐츠 유형별, 채널별, 시간대별, 해시태그별, 캠페인별 성과 분해 제공
- 세부 분석은 모든 패널을 한 번에 나열하지 않고 탭 전환형 drill-down으로 제공
...
### Phase 4. Brand Hub 전환

- [x] Brand Hub를 브랜드 공용 설정 화면으로 전환
...
- [x] 기본 화면에서 꼭 필요한 작업만 먼저 보이고, 보조 정보는 접거나 탭으로 분리된다.
- [x] 브랜드 자산/지식/분석 화면 모두 과도한 세로 스크롤을 줄이도록 내부 스크롤과 요약 레이아웃을 사용한다.
...
5. 실제 자산 수가 많은 브랜드 기준으로 Brand Studio 내부 스크롤 한계와 필터 사용성을 재검증
