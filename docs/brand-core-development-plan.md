...
- [ ] 브랜드 조회/생성/수정 서비스 추가
- [ ] 에피소드 생성 시 `brandId` 연결 구조 추가
- [ ] 공용 자산 조회 서비스 추가
- [ ] 브랜드 단위 게시 결과/분석 데이터 저장 경로 추가
...
- [ ] Brand Studio 진입 기준을 `projectId` 중심에서 `brandId` 중심으로 전환
- [ ] 브랜드 하위 전체 자산 선택 UI 추가
- [ ] 영상/이미지/글/게시물 초안을 한 화면에서 선택 가능하게 구성
- [ ] 채널 연결을 브랜드 공용 설정으로 이동
- [ ] 예약 게시와 게시 결과를 브랜드 기준으로 저장
...
- [ ] Knowledge Hub를 브랜드 공용 설정 화면으로 전환
- [ ] 브랜드 보이스, 규칙, 금지 표현, 성공 사례를 브랜드 레벨에 저장
- [ ] 새 에피소드 생성 시 Knowledge 자동 연결
- [ ] 에피소드 개별 override 허용 범위 정의
...
- [ ] 새 에피소드 생성 시 브랜드 문맥 재입력이 필요 없다.
- [ ] Brand Studio에서 여러 에피소드 자산을 함께 선택할 수 있다.
- [ ] 캡션/해시태그 기본값이 브랜드 문맥을 먼저 반영한다.
- [ ] 분석 첫 화면에서 브랜드 전체 상태를 바로 이해할 수 있다.
- [ ] 분석 화면에서 에피소드/콘텐츠 유형/채널/시간대별 성과를 쉽게 비교할 수 있다.
