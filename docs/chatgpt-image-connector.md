...
랜딩페이지 `구독·API 설정`에서 `이미지 생성 — 내 구독·API 사용`을 체크하고 `내 ChatGPT 구독`을 선택하면 NKStudio의 모든 이미지 생성 구간이 사용자 PC의 공식 Codex app-server로 생성하고 자동 저장합니다. AI 기업 에이전트·승인 실행과 영상 제작 중의 스틸 생성도 포함합니다. 체크를 해제하면 마스터 설정을 사용합니다. 본인 인증 오류를 마스터 과금으로 전환하지 않습니다. 첫 연결과 ChatGPT 로그인은 사용자가 직접 확인합니다.
...
5. 랜딩페이지에서 이미지 본인 사용 체크와 `내 ChatGPT 구독` 선택을 확인하고 저장합니다. 어떤 이미지 생성 화면이나 에이전트에서 생성해도 본인 구독을 사용합니다. 결과는 세션/프로젝트 저장소에 자동 저장되며 AI 기업은 저장 완료 후 검수·업무 파일에 표시합니다.
...
- 여러 이미지 요청은 큐에 접수하고 소유자당 한 번에 하나씩 실행합니다. 동일 요청 ID와 결과 업로드 재전송은 같은 작업을 사용합니다. 연결 오류·사용 한도 오류로 이미지 생성을 자동 반복하지 않습니다.
