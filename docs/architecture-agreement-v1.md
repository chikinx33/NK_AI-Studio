...
현재 구조에서 `Project`는 제작과 저장의 기본 컨테이너이고, `Brand`는 운영과 분석의 기본 문맥이다.

즉 사용자는 에피소드/프로젝트를 만들지만, 운영 화면에서는 브랜드 기준으로 자산을 묶어 보고 배포와 분석을 이어간다.
...
Project Core는 제작 흐름의 루트이며 모든 기능이 공유한다.
...
- 사용자는 지금 보는 화면이 `브랜드 운영 화면`인지 `에피소드 제작 화면`인지 즉시 구분할 수 있어야 한다.
...
- 기본 화면은 스크롤과 입력 부담을 최소화해야 하며, 보조 정보와 상세 설정은 접거나 탭으로 분리한다.
